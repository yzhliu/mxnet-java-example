...
public class Seq2SeqInference {
  public static void main(String[] args) {
    ...
    String output = NDArrayCollector.auto().withScope(new scala.runtime.AbstractFunction0<String>() {
      @Override
      public String apply() {
        ...
        if (input.length < MAX_LENGTH) {
          encoderOutputs = NDArray.concat(ScalaConverter.convert(
              NDArray.flatten(ScalaConverter.convert(encoderOutputs)),
              NDArray.zeros(Shape.create(MAX_LENGTH - input.length, HIDDEN_SIZE), Context.cpu(0), DType.Float32()),
              2, 0)).get();
        } else {
          encoderOutputs = NDArray.flatten(ScalaConverter.convert(encoderOutputs)).get();
        }
        ...
        for (int i = 0; i < 50; ++i) {
          ...
          NDArray topi = NDArray.argmax(ScalaConverter.convert(decoderOutput, 1)).get();
